...
| `{slug}` | 없음 | 슬러그화된 제목 | `사용자-인증`, `api-design` |
...
| 커맨드 | 설명 |
|--------|------|
| `/docs-new` | 새 번호 매김 문서 생성 |
| `/docs-migrate` | 기존 문서를 번호 매김 규칙으로 변환 |
| `/docs-rollback` | 마지막 docs-numbering 작업 되돌리기 |

### 스킬 자동 트리거

`docs-numbering` 스킬은 다음 상황에서 자동 활성화됩니다:
- 마크다운 문서 저장/생성 요청
- 방법론 단계 완료 시 산출물 생성
- 문서 정리, 번호 매기기 요청
- "번호 매겨줘", "정리해줘" 등의 한국어 요청
